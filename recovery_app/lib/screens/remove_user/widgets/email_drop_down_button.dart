...
class EmailDropDownButton extends StatefulWidget {
  final String title;
  const EmailDropDownButton({super.key, required this.title});
...
class _EmailDropDownButtonState extends State<EmailDropDownButton> {
  ...
  void didChangeDependencies() async {
    ...
    setState(() {
      final list = authProvider.getUserEmails;
      if (list.isEmpty) {
        items.clear();
      } else {
        items = list;
  ...
  Widget build(BuildContext context) {
    ...
    try {
      return DropdownButtonHideUnderline(
        child: DropdownButton2<String>(
          ...
          onChanged: (value) {
            ...
            authProvider.setUserEmailToBeDeleted(selectedValue as String);
          },
      ...
      return FittedBox(child: Text("Error Occured. Go back"));
    }
  }
}
